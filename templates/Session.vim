...
nmap ,P Op==
nmap ,p op==
nmap ,m :call cursor(0, len(getline('.'))/2)
nmap ,v :e $MYVIMRC
nmap ,s :source $MYVIMRC
noremap ,w :update
vmap ,z :%s/\<*\>/
nmap ,z :%s#\<=expand("<cword>")\>#
map P :TlistToggle
...
nnoremap <F3> :set nonumber!
nnoremap <F2> :set invpaste paste?
imap S <Plug>ISurround
imap s <Plug>Isurround
inoremap <silent> 	 =TriggerSnippet()
inoremap <NL> =Tab_Or_Complete()
inoremap  
...
set softtabstop=4
set nostartofline
set statusline=%f-%{synIDattr(synID(line('.'),col('.'),1),'name')}
...
silent only
cd ~/django/herokus/spreadsheet/templates
if expand('%') == '' && !&modified && line('$') <= 1 && getline(1) == ''
  let s:wipebuf = bufnr('%')
endif
set shortmess=aoO
badd +0 sheets/index.html
args sheets/index.html
edit sheets/index.html
